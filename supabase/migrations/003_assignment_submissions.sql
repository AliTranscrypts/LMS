...
DROP POLICY IF EXISTS "Teachers can read submission files in their courses" ON storage.objects;
...
WITH CHECK (
  ...
  (storage.foldername(storage.objects.name))[1] = 'submissions' AND
  -- The path format is: submissions/{assignment_id}/{student_id}/{timestamp}.{ext}
  (storage.foldername(storage.objects.name))[3] = auth.uid()::text AND
  ...
  EXISTS (
    ...
    WHERE ct.id = ((storage.foldername(storage.objects.name))[2])::uuid
